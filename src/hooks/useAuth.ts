...
export function useAuth(): AuthState & AuthActions {
    const { isAuthenticated, user, sessionToken, login: storeLogin, logout: storeLogout } = useAuthStore();
    ...
    const login = useCallback(async (email: string): Promise<void> => {
        ...
        try {
            ...
            if (result.stage === 'verify') {
                ...
                const { user, sessionToken } = await authService.validateSessionWithBackend();
     ...
     * Verify new account
     */
    const verifyAccount = useCallback(async (verificationCode: string): Promise<void> => {
        setIsLoading(true);
        setError(null);
        ...
        try {
            ...
            const result = await authService.verifyNewAccount(verificationCode);
            ...
            if (result.success) {
                ...
                logger.info('User account verified and authenticated', { userId: user.id });
            } else {
                throw new Error('Verification failed');
            }
        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Verification failed';
            logger.error('Account verification failed', { error: errorMessage });
            setError(errorMessage);
            throw error;
        } finally {
            setIsLoading(false);
...
export function useAuthActions(): AuthActions {
    const { login, verifyAccount, logout, refreshSession, clearError } = useAuth();
