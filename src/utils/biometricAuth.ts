...
export class BiometricAuth {
    ...
    async isAvailable(): Promise<boolean> {
        try {
            ...
            logger.info('Biometric availability check', {
                hasHardware,
                isEnrolled,
            });
    ...
    async isEnabled(): Promise<boolean> {
        try {
            const enabled = await SecureStorage.getSecureItem(
    ...
    async disable(): Promise<void> {
        try {
            await SecureStorage.removeSecureItem(
                STORAGE_KEYS.BIOMETRIC_ENABLED
            );
            logger.info('Biometric authentication disabled');
        } catch (error) {
            logger.error('Failed to disable biometric authentication', {
                error,
            });
        }
    }
     ...
     */
    private getBiometricTypeFromResult(
        result: LocalAuthentication.LocalAuthenticationResult
    ): 'fingerprint' | 'facial' | 'iris' | 'voice' | undefined {
...
export class TransactionBiometricAuth {
     ...
     */
    static async authenticateForTransaction(
        amount: number,
        currency: string
    ): Promise<BiometricAuthResult> {
     ...
     */
    static async authenticateForAddMoney(
        amount: number,
        currency: string
    ): Promise<BiometricAuthResult> {
     ...
     */
    static async authenticateForCashOut(
        amount: number,
        currency: string
    ): Promise<BiometricAuthResult> {
