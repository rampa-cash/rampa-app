...
function resolveColors(
    ...
    const trackOff =
        overrides?.trackColorInactive ??
        (mode === 'dark' ? t.outline.outline2 : t.outline.outline2);
...
export function Toggle({
    ...
    const colors = useMemo(
        () =>
            resolveColors(
                isDark ? ('dark' as Mode) : ('light' as Mode),
                current,
                t,
                disabled,
                {
    ...
    const labelNode =
        typeof label === 'string' ? (
            <Text
                style={[
                    styles.label,
                    { color: disabled ? t.text.lessEmphasis : t.text.normal },
                ]}
            >
                {label}
            </Text>
