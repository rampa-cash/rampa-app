  ...
  return (
    ...
    >
      ...
      <View style={{ flex: 1, gap: 6}}>
        <AppText variant={TextVariant.BodyMedium}>{contact.name}</AppText>
        {Boolean(contact.phone) && (
          <AppText variant={TextVariant.BodyMedium} color={'lessEmphasis' as any}>
            {contact.phone}
          </AppText>
        )}
      </View>
      {showInviteButton ? (
        <AppButton title="Invite" variant={ButtonVariant.Tertiary} onPress={() => onInvite?.(contact.id)} color='normal' />
      ) : rightAccessory ? (
