...
export default function SendSuccessScreen() {
  ...
  const { amount = '0', currency = 'EUR', contactName = 'Recipient', contactRef = '' } =
  ...
  return (
    <ScreenContainer padded scroll contentContainerStyle={[styles.container, { paddingBottom: insets.bottom + 24 }]}>
      <View style={styles.center}>
        ...
        <AppText variant={TextVariant.Body} style={{ marginTop: 12 }}>Sent!</AppText>
        ...
        {Boolean(contactRef) && (
          <AppText variant={TextVariant.Caption} color={'lessEmphasis' as any} align="center">
            {String(contactRef)}
          </AppText>
        )}

        <Pressable onPress={() => { /* TODO: navigate to tx details */ }}>
      ...
      <View style={{ marginTop: 32 }}>
        <AppButton title="Close" variant={ButtonVariant.Primary} onPress={() => router.replace('/(tabs)/home' as any)} />
      </View>
    </ScreenContainer>
  );
}
